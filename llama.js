...
function upgradeNum13(cost, reload = false) {
	if (upgradeSetter(cost, 13)) {
		...
		rateDoubler(pastures, pastures.multiplier);
	}
}
...
///////////////////////////
////////// Fading /////////
function fadeAssister() {
	fadeOut($('#saved'));
	$("#saved").css("display", "block");
	$("#saved").css("innerHTML", "Status:");
}
...
function humClick(number, repetition = false) {
	if (!repetition) {hums += humMultiplier * number;}
	else {hums += number}
	$('#hums').html(nFormatter(hums, 3));
...
function toggleSound(autoLoad = false) {
	if (!autoLoad) soundsOn = !soundsOn;
	$("#toggleSound").toggleClass("buttonOn buttonOff");
}
/////////////////////////////////////////////
...
function saveGame(manualSaved = true) {
	...
	var inside = manualSaved ? "Status: Saved" : "Status: Autosaved";
	savedElement.innerHTML = inside;
	window.setTimeout(fadeAssister, 5000);
}
...
function deleteGame() {
	...
	var savedElement = document.getElementById('saved');
	fadeIn(savedElement);
	savedElement.innerHTML = "Status: Deleted";
	window.setTimeout(fadeAssister, 5000);
}
//////////////////////////////
